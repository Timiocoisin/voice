...
from client.api_client import (
    ...
    get_diamond_balance as api_get_diamond_balance,
    get_user_profile as api_get_user_profile,
)
...
class VipPackageDialog(BaseDialog):
    ...
    def _load_vip_info(self):
        ...
        if not self.user_id:
            return
        vip = api_get_vip_info(self.user_id) or {}
        expiry_str = vip.get("vip_expiry_date")
        if not expiry_str:
            return
        try:
            self.vip_expiry = datetime.fromisoformat(expiry_str)
        except Exception:
            self.vip_expiry = None
    ...
    def _on_buy_membership(self, card_info: Dict):
        ...
        vip = api_get_vip_info(self.user_id) or {}
        diamonds = int(vip.get("diamonds", 0))
        ...
        # 调用后端接口：扣减钻石并更新 VIP
        resp = api_purchase_membership(self.user_id, card_info)
        if not resp.get("success", False):
            msg_box = CustomMessageBox(self.parent(), variant="error")
            msg_box.setWindowTitle(text_cfg.VIP_PURCHASE_FAILED_TITLE)
            msg_box.setText(resp.get("message", text_cfg.VIP_PURCHASE_FAILED_MESSAGE))
            msg_box.exec()
            return
...
class DiamondPackageDialog(BaseDialog):
    ...
    def _load_user_avatar(self) -> QPixmap:
        ...
        # 从后端获取用户资料与头像
        profile = api_get_user_profile(self.user_id)
        if not profile:
            avatar_bytes = get_default_avatar()
            pix = QPixmap()
            if avatar_bytes and pix.loadFromData(avatar_bytes):
    ...
    def _load_user_diamond_balance(self):
        ...
        if not self.user_id:
            self.diamond_balance = 0
            return
        diamonds = api_get_diamond_balance(self.user_id) or 0
        self.diamond_balance = diamonds
